          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              >
                ...
                >
                  Browse Staffly Agency & get matched with local & international clients.
                </motion.p>
        ...
        <section className="py-16 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            ...
            >
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">Browse our current openings and apply to the role that fits you best.</p>
            </motion.div>

            <div className="grid md:grid-cols-2 gap-6">
              {[
                { title: 'Executive Assistant (US Hours)', location: 'Remote • PH-based', type: 'Full-time', tags: ['Operations', 'Scheduling', 'Inbox'], link: '/candidate/signup' },
                { title: 'Appointment Setter (Agency)', location: 'Remote • PH-based', type: 'Full-time', tags: ['Outbound', 'CRM', 'Email/SMS'], link: '/candidate/signup' },
                { title: 'Customer Support Specialist', location: 'Remote • PH-based', type: 'Full-time', tags: ['Tickets', 'SLAs', 'Zendesk'], link: '/candidate/signup' },
                { title: 'Project Coordinator', location: 'Remote • PH-based', type: 'Full-time', tags: ['Timelines', 'Docs', 'Asana'], link: '/candidate/signup' },
              ].map((job, idx) => (
                <motion.div key={idx}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="text-sm text-gray-600 mt-1">{job.location} • {job.type}</div>
                    </div>
                    <Link to={job.link} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Apply</Link>
                  </div>
                  <div className="flex flex-wrap gap-2 mt-4">
                    {job.tags.map((t) => (
                      <span key={t} className="px-3 py-1 text-xs bg-gray-100 text-gray-700 rounded-full">{t}</span>
                    ))}
                  </div>
                </motion.div>
              ))}
