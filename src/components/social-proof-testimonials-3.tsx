...
export const TestimonialCard = ({
  ...
  <div
    className={cn(
      "mb-4 flex w-full cursor-pointer break-inside-avoid flex-col items-center justify-between gap-6 rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow",
      // light styles
  ...
  >
    <div className="text-sm font-normal text-slate-700 select-none dark:text-neutral-400">
      {description}
    ...
    <div className="flex w-full items-center justify-start gap-4 select-none">
      ...
      <div>
        ...
        <p className="text-sm font-normal text-slate-600">{role}</p>
      </div>
    </div>
  </div>
)
          ...
          <div className="relative mt-6 overflow-hidden">
            ...
            >
              {testimonials.map((card, idx) => (
                <TestimonialCard {...card} key={idx} className="mx-4 w-[400px]" />
              ))}
            </Marquee>
            <div className="pointer-events-none absolute inset-y-0 left-0 w-1/4 bg-gradient-to-r from-white from-10%"></div>
            <div className="pointer-events-none absolute inset-y-0 right-0 w-1/4 bg-gradient-to-l from-white from-10%"></div>
          </div>
        </div>
      </div>
    </section>
